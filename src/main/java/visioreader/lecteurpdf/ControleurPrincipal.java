...
public class ControleurPrincipal implements Initializable {
    ...
    public void zoomVue(Vue vue) {
        ...
        if (nbVues == 1) { // 1 vue sur la fenêtre -> largeur max de la vue -> largeur de la fenêtre
            vue.getControleur().getImageAfficher().setPreserveRatio(false);
            vue.getControleur().getImageAfficher().setFitWidth(tab[fenetre-1].getWidth()*0.99); // 0.99 --> Ajustement par rapport au SplitPane
            vue.getControleur().getImageAfficher().setPreserveRatio(true);

        } else { // 2 vues sur la fenêtre -> largeur max de la vue -> dépend de la positio du divider
            /*
             * Pour faire le zoom automatique on récupère la position du divider. ( comprise entre 0 et 1 ( pourcentage ) )
             *   Pour la vue de gauche : On multiplie la largeur de la fenêtre par la position du divider
             *   Pour la vue de droite : On multiplie la largeur de la fenêtre par 1 - la position du divider
             * Ceci est le facteur.
             */

            double facteur; // Le facteur par lequel on multiplie

            facteur = vue.getEmplacement().getPosition() == 1 ? tab[fenetre-1].getDividerPositions()[0] : 1-tab[fenetre-1].getDividerPositions()[0];

            vue.getControleur().getImageAfficher().setPreserveRatio(false);
            vue.getControleur().getImageAfficher().setFitWidth(tab[fenetre-1].getWidth()*facteur*0.99);
            vue.getControleur().getImageAfficher().setPreserveRatio(true);
        }
    }
